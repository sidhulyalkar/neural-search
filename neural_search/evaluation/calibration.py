...
def compute_reliability_bins(
    ...
    label_list = [int(bool(l)) for l in labels]
    ...
    for bin_idx in range(len(bin_edges) - 1):
        ...
        for conf, label in zip(conf_list, label_list):
            if is_last:
...
def compute_calibration_metrics(
    ...
    # Brier score
    brier = sum((c - l) ** 2 for c, l in zip(conf_list, label_list)) / n
    ...
    overconf = sum(1 for c, l in zip(conf_list, label_list) if c > l) / n
    underconf = sum(1 for c, l in zip(conf_list, label_list) if c < l) / n
...
def _compute_calibration_slope(
    ...
    sum_y = sum(labels)
    sum_xy = sum(c * l for c, l in zip(confidences, labels))
