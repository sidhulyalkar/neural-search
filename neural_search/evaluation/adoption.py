...
class AdoptionEvent:
    ...
    def validate(self) -> None:
        ...
        if self.usefulness not in {None, "useful", "partially_useful", "not_useful", "unsure"}:
    ...
    with path.open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            try:
                ...
                raise ValueError(f"Invalid adoption event at line {line_number}: {exc}") from exc
    return events
...
def evaluate_adoption_events(events: Iterable[AdoptionEvent]) -> dict[str, Any]:
    ...
    positive_reuse_decisions = [
        event for event in reuse_decisions if event.would_use_for_analysis in {"yes", "maybe"}
    ]
    useful_events = [
        event
        for event in event_list
    ...
    novel_useful = [
        event for event in useful_events if event.known_before is False
    ]
    ...
    for session_events in by_session.values():
        session_events = sorted(session_events, key=lambda event: _parse_timestamp(event.timestamp))
        ...
        if any(event.event_type == "workflow_complete" and _event_success(event) for event in session_events):
        ...
        if failure_indexes:
            ...
            if any(
                any(
                    subsequent.event_type == "search_success" and _event_success(subsequent)
    ...
    first_search_median = (
        round(statistics.median(session_first_search_seconds), 3)
        if session_first_search_seconds
        else None
    )
    ...
    return {
        ...
        "metrics": {
            ...
            "save_rate_per_session": _ratio(len(saved), total_sessions),
            ...
            "time_to_first_successful_search_seconds": {
                "median": first_search_median,
                "p90": _percentile(session_first_search_seconds, 0.9),
                "observations": len(session_first_search_seconds),
            },
        },
        "counts": {
        ...
        "interpretation": {
            ...
            "note": (
                "These metrics measure usability and downstream research workflow completion. "
                "They do not replace NDCG/MRR/qrels evaluation or validate scientific conclusions."
            ),
        },
    }
