# Neural Search Dataset Compilation Report

Generated: 2026-05-23T03:39:39.621584+00:00

## Summary

- **Total datasets**: 5
- **Total linked papers**: 5
- **Dataset cards generated**: 5
- **Average analysis readiness**: 95.0/100
- **Datasets with behavior**: 5
- **Datasets with trial structure**: 5
- **Datasets with linked papers**: 5

## Curated Sources (Pending Ingestion)

- Total curated entries: 10
- Datasets: 8
- Papers: 2

### By Source Type

- dandi: 3
- openneuro: 3
- openalex: 2
- manual: 2

### By Priority

- high: 7
- medium: 3

## Datasets by Source

- demo: 5

## Datasets by Task

- go_nogo: 1
- reversal_learning: 1
- delay_discounting: 1
- reaching: 1
- center_out_reaching: 1
- grasping: 1
- visual_decision_making: 1
- visual_discrimination: 1

## Datasets by Modality

- behavior_video: 2
- extracellular_ephys: 2
- calcium_imaging: 1
- fiber_photometry: 1
- ecog: 1
- ieeg: 1
- pose_tracking: 1
- bci: 1
- neuropixels: 1

## Datasets by Species

- mouse: 3
- rat: 1
- human: 1

## Datasets by Brain Region

- mPFC: 2
- OFC: 1
- striatum: 1
- nucleus_accumbens: 1
- motor_cortex: 1
- somatosensory_cortex: 1
- parietal_cortex: 1
- visual_cortex: 1
- PPC: 1

## Datasets by Data Standard

- NWB: 3
- BIDS: 2

## Behavioral Coverage

- Datasets with behavior: 5 (100%)
- Datasets with trial/event structure: 5 (100%)

## Top 20 Analysis-Ready Datasets

| Rank | Source ID | Title | Score | Source |
|------|-----------|-------|-------|--------|
| 1 | DEMO_GONOGO_CALCIUM | Demo Go/NoGo calcium imaging dataset | 95 | demo |
| 2 | DEMO_REVERSAL_EPHYS | Demo reversal learning electrophysiology... | 95 | demo |
| 3 | DEMO_DELAY_DISCOUNTING | Demo delay discounting behavior and neur... | 95 | demo |
| 4 | DEMO_REACHING_ECOG_IEEG | Demo reaching ECoG and iEEG dataset | 95 | demo |
| 5 | DEMO_VISUAL_DECISION_NEUROPIXELS | Demo visual decision-making Neuropixels ... | 95 | demo |

## Missing Metadata Summary

- Datasets with incomplete metadata: 0

### Fields Most Often Missing


### Distribution by Missing Field Count

