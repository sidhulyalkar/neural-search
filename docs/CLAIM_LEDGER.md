...
| Paper-dataset linking improves search quality | partially_implemented | 393 papers linked via OpenAlex (168 DOI-exact + 225 title-fuzzy) out of 7,171 corpus datasets | Low coverage (6,778 not-found), no precision audit yet (template exists, blank) | Expand coverage, fill `reports/eval/paper_link_audit_template.csv`, add linkage ablation |
| Graph features improve ranking | prototype_validated | Ablation shows -4.6% NDCG without graph (older concept-memory ablation, `reports/ablation_v08.md`); on the canonical 317-query ladder, `hybrid_graph`/`full` lead BM25 by +0.013 NDCG@10 (0.6696 vs 0.6566) but CIs overlap substantially ([0.6453,0.6921] vs [0.6322,0.6807]) — not yet statistically established | Two different ablations, neither human-adjudicated; small/unproven effect size either way | Add the typed-KG layer (next row) as its own rung to isolate it from the aggregate graph signal; human-adjudicate a subset |
| Typed/relationship finding edges improve retrieval | prototype_validated (negative/null result) | `reports/eval/relationship_edge_quality.md`: edge-type promotions sit at 0.473–0.522 helpful rate among judged top-10 promotions — close to a coin flip. **New (2026-06-23):** dedicated `typed_kg`/`typed_kg_qualified` ablation rungs (`neural_search/graph/typed_kg_features.py`) isolate the typed-relationship signal from the aggregate `hybrid_graph` score; on a full-corpus run (`reports/eval/ndcg_report_full_corpus.md`), `typed_kg` was numerically identical to `hybrid_rrf` (no measurable NDCG@10 change), and `typed_kg_qualified` moved NDCG@10 by only +0.0002 | Full-corpus run isn't the canonical packet-corpus benchmark (different corpus/graph scale; not directly comparable to the committed `ndcg_report.md` numbers); the calibrated weight (0.005) may simply be too small at this corpus size, not necessarily evidence the signal is valueless | Re-run `typed_kg`/`typed_kg_qualified` against the canonical packet-aligned corpus for a true apples-to-apples comparison; if still null, do not invest further in relationship-edge expansion (Tasks 3–6 of the typed-field-coverage plan) without human-adjudicated qrels first |
| Typed-field extraction (negation/frequency_band/temporal_pattern) is precise enough to drive contradiction detection | prototype_validated | **New (2026-06-24):** `reports/eval/typed_field_audit_summary_2026.md` — 32-row audit found 46.9%/53.1% precision (below the 60% threshold), dominated by `negation` false positives (bare `inhibit`/`suppress`/`block` patterns matching affirmative findings) and `frequency_band` collisions with molecule names (`amyloid-beta`, `alpha-lipoic acid`). Fixed same day: 90.6%/93.75% precision on re-extraction of the same 32 rows, 16 new regression tests. Also discovered and fixed a structural gap: `relationship_builder.py` had never actually been fed typed-field-enriched input (`findings_tier1_normalized.jsonl` predated the extractor's integration), so `direct_refutation` contradictions had always been 0 regardless of the regex bugs. Re-ran the full pipeline at 190,251-finding scale: `direct_refutation` is now 9,833 (was 0), strong consensus records 274 (was 34) | Still LLM-silver-extracted evidence (82%/90% precision on the underlying finding extraction itself, see `finding_audit_llm_judge_summary_2026.md`) — not human-validated at any layer | Human-audit a sample of the new `direct_refutation` edges specifically (different failure surface than the base finding-extraction audit); consider whether `relationship_edge_quality.md`'s coin-flip retrieval-helpfulness result should be re-measured against this corrected, full-scale data |
