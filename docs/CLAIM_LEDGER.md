...
| Typed/relationship finding edges improve retrieval | not_started | `reports/eval/relationship_edge_quality.md`: edge-type promotions sit at 0.473–0.522 helpful rate among judged top-10 promotions — close to a coin flip, no edge type clearly beats `no_relationship_edge` baseline (0.333, n=39) | Analysis predates the 2026-06-23 Phase 6b qualified-consensus/contradiction-subtype edge types; only tested as promotions within `hybrid_graph`, not as an isolated ablation rung | Add dedicated `typed_kg`/`typed_kg_qualified` ablation rungs against the canonical qrels before investing further in relationship-edge expansion (Tasks 3–6 of the typed-field-coverage plan) |
